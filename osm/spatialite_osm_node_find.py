...
class spatialite_osm_node_finder(handler.ContentHandler):
    ...
    # function: return a "osm id" within the ThresholdMinDist distance
    ...
    #################################################################
    # Input: - latutude, longitude of the two points (lat1, lon1) and (lat2, lon2)  		#
    ...
    def route_distance_node_id(self, from_node_id, to_node_id):
        if from_node_id != None and to_node_id != None:
            #            distance_query = "SELECT  ST_Length(geometry) AS lenght FROM roads_net WHERE NodeFrom=%s AND NodeTo=%s;" % (from_node_id, to_node_id)
            distance_query = "SELECT * FROM roads_net WHERE NodeFrom=%s AND NodeTo=%s;" % (from_node_id, to_node_id)
            self.cur.execute(distance_query)
            route = self.cur.fetchone()
            # printing when successful
        if route != None:
    ...
    def find_grid_point_node_id2(self, latitude, longitude,
                                 ThresholdMinDist=0.010):  # consider the node found if it's in range of 100 meter range
        maxDist = 1000
        # for efficiency, if a node is closer
        nodeFound = None
        ...
        if grid_number == None:
            ...
            for id in self.gridDictionary[grid_number].keys():
                ...
                if dist <= ThresholdMinDist:
                    ...
                    # print "from Threshold"
                    # return(nodeFound)
                    return self.osm_id_node_id[nodeFound], self.nodes[nodeFound]
        if nodeFound != None:
            return self.osm_id_node_id[nodeFound], self.nodes[nodeFound]
        else:
            return None, None